...
def parse_args():
    ...
    parser.add_argument('--checkpoint', default='./pretrained/Vit-L_ADE20k_54.6.pth')
    parser.add_argument('--lr', default=1e-3, type=float, help='learning rate')
    parser.add_argument('--batch_size', default=4, type=int, help='batch size')
    parser.add_argument('--cuda', default=True, action='store_true')  
    parser.add_argument('--device', default="cuda", type=str, help='pytorch device')
    parser.add_argument('--epochs', default=20, type=int, help='number of epochs to trainkj')  
    ...
    parser.add_argument('--teach_w', default=0, type=float, help='the loss weight of teacher label')
    parser.add_argument('--ground_w', default=1, type=float, help='the loss weight of ground truth')
    parser.add_argument('--num_class', default=151, type=int, help='number of classes')
    parser.add_argument('--amp', default=True, action='store_true', help='enabled mixed precision')
    ...
    with amp.autocast(enabled=args.amp):
        with torch.no_grad():
            ...
            if not isinstance(teacher_pred, dict):
                ...
                B, C, H, W = img.shape
                teacher_pred = torch.cat([torch.zeros((B, 1, H, W),dtype=torch.float,device=args.device), teacher_pred], dim=1)
                ...
                teacher_pred[:, 1:, :, :] = ~mask.to(torch.int) * teacher_pred[:, 1:, :, :]
            else:
...
def train_baseline(student, img, label, optimizer, use_stages, args):
    with amp.autocast(enabled=args.amp):
        if isinstance(student, tuple):
            student1, student2 = student
            student1_pred = student1(img)
            ...
            if args.model_type != 'simple_segvit':
                student_pred = student_pred.permute(0, 2, 3, 1).contiguous().view(-1, 151)
                label = label.permute(0, 2, 3, 1).contiguous().view(-1)
                loss = F.cross_entropy(student_pred, label)
            else:
                student_pred = student(img)
                ...
                for i in range(args.batch_size):
                    ...
                    for j in range(len(student_pred['aux_outputs'])):
                        ...
                        dict1['pred_masks'] = student_pred['aux_outputs'][j]['pred_masks'][i].unsqueeze(0)
                        tmp['aux_outputs'].append(dict1)
                    losses = atm_loss(tmp, label[i], ignore_index=150)
                    ground_losses += (losses['loss_ce'] + losses['loss_mask'] + losses['loss_dice'])
                loss = ground_losses/args.batch_size
...
def main():
    ...
    if args.model_dir != '':
        ...
        args.log_dir = f'{args.log_dir}/{args.model_type}/{now_time}'
        epochs = args.epochs
        start_epoch = 1
    ...
    if args.model_dir != '':
        ...
        student = AttUNet().to(args.device)
    elif args.model_type == 'Unet':
        student = UNet().to(args.device)
        ...
        student1 = AttUNet().to(args.device)
    ...
    if args.teach_w > 0:
        ...
        teacher = teacher.eval().to(args.device)
    ...
    if args.model_type == 'fuse':
        optimizer = optimizer([student1.parameters()] + [student2.parameters()], lr=args.lr)
        best_test_mIoU1 = best_test_mIoU2 = 0
    else:
        optimizer = optimizer(student.parameters(), lr=args.lr)
        best_test_mIoU = 0
        ...
        for img, label in tqdm(train_loader):
            ...
            if args.model_type == 'fuse':
                ...
                if teacher != None:
                    ...
                    loss = train_baseline(student, img, label, optimizer, use_stages, args)
                epoch_loss += loss
