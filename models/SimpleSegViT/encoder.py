...
class ViTEncoder(VisionTransformer):
    ...
    def forward(self, inputs: Tensor):
        ...
        x, hw_shape = self.patch_embed(inputs) # x expected shape (B, L, C) , hw_shape is tuple (H / patch_size, W / patch_sizeh) 

        # stole cls_tokens impl from Phil Wang, thanks
        cls_tokens = self.cls_token.expand(B, -1, -1) # (B, 1, C)
        x = torch.cat((cls_tokens, x), dim=1) # (B, L + 1, C)
        x = self._pos_embeding(x, hw_shape, self.pos_embed) # (B, L + 1, C)
